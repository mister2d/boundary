...
func toProto(in credential.Library, opt ...handlers.Option) (*pb.CredentialLibrary, error) {
	...
	case vault.Subtype:
		...
		if outputFields.Has(globals.CredentialTypeField) && vaultIn.GetCredentialType() != string(credential.UnspecifiedType) {
			...
			if outputFields.Has(globals.CredentialMappingOverridesField) && vaultIn.GetMapping() != nil {
				m := make(map[string]interface{})
				switch mapping := vaultIn.GetMapping().(type) {
				case *vault.UserPasswordOverride:
					// TODO: should domain remove sentinels?
					if mapping.UsernameAttribute != "" && mapping.UsernameAttribute != sentinel.NoOverride {
						m[string(credential.UsernameField)] = mapping.UsernameAttribute
					}
					if mapping.PasswordAttribute != "" && mapping.PasswordAttribute != sentinel.NoOverride {
						m[string(credential.PasswordField)] = mapping.PasswordAttribute
					}
				}
				if len(m) > 0 {
...
func toStorageVaultLibrary(storeId string, in *pb.CredentialLibrary) (out *vault.CredentialLibrary, err error) {
	...
	case credential.UserPasswordType:
		...
		if username := overrides[string(credential.UsernameField)]; username != nil {
			mapOpts = append(mapOpts, vault.WithOverrideUsernameAttribute(username.(string)))
		}
		if password := overrides[string(credential.PasswordField)]; password != nil {
